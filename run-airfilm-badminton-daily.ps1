...
while ($true) {
  ...
  $now = Get-Date
  if ($now -lt $launchAt) {
    $wait = [int]($launchAt - $now).TotalSeconds
    Write-Host "Next airfilm badminton run: $noon. Browser opens at $launchAt."
    Start-Sleep -Seconds $wait
  }

  Write-Host "Opening Edge. Complete SJTU login/captcha before $noon if prompted."
  & "$root\launch-edge-sjtu.ps1"
  Wait-EdgeDebugPort

  $now = Get-Date
  if ($now -lt $noon) {
    $wait = [int]($noon - $now).TotalSeconds
    Start-Sleep -Seconds $wait
  }

  Write-Host "Running airfilm badminton booking preparation at $(Get-Date)."
  $env:TARGET_VENUE_ID = "3b10ff47-7e83-4c21-816c-5edc257168c1"
  $env:TARGET_DATE = ""
  $env:TARGET_DATE_MODE = "last"
  $env:TARGET_TYPE_CODE = "badminton"
  $env:TARGET_TYPE = ""
  $env:TIME_ROWS = "13,14"
  $env:TARGET_FIELD = ""
  $env:FORCE_REFRESH = "1"
